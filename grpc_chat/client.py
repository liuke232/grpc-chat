...
try:
    ...
    raise ImportError(
        "tkinter模块未找到。请确保您的Python安装包含tkinter。\n"
        "在Ubuntu/Debian上，可以运行: sudo apt-get install python3-tk\n"
        "在CentOS/RHEL上，可以运行: sudo yum install tkinter\n"
        "在macOS上，请重新安装Python并确保包含tkinter。"
    )
...
# 这里IDE可能会对chat_pb2和chat_pb2_grpc的import报错，忽略即可
...
class ChatClientGUI:
    ...
    def create_login_window(self):
        ...
        title_label = ttk.Label(
            main_frame, text="欢迎使用分布式聊天系统", font=("Arial", 16, "bold")
        )
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 20))

        # 用户名输入
        ttk.Label(
            main_frame, text="请输入用户名 (1-20个字母或数字):", font=("Arial", 10)
        ).grid(row=1, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))

        self.username_var = tk.StringVar()
        username_entry = ttk.Entry(
            main_frame, textvariable=self.username_var, font=("Arial", 11), width=30
        )
        username_entry.grid(row=2, column=0, columnspan=2, pady=(0, 20))

        # 登录按钮
        login_button = ttk.Button(main_frame, text="登录", command=self.handle_login)
        login_button.grid(row=3, column=0, columnspan=2, pady=(0, 10))

        # 退出按钮
        quit_button = ttk.Button(main_frame, text="退出", command=self.quit_application)
    ...
    def show_lobby(self):
        ...
        self.room_listbox = tk.Listbox(
            main_frame, width=40, height=15, font=("Arial", 11)
        )
        ...
        # 刷新按钮
        ttk.Button(button_frame, text="刷新列表", command=self.refresh_rooms).pack(
            side=tk.LEFT, padx=5
        )

        # 加入按钮
        ttk.Button(button_frame, text="加入房间", command=self.join_selected_room).pack(
            side=tk.LEFT, padx=5
        )

        # 退出按钮
        ttk.Button(button_frame, text="退出登录", command=self.handle_logout).pack(
            side=tk.LEFT, padx=5
        )

        # 状态标签（新增）
        self.lobby_status_label = ttk.Label(
            main_frame, text="请选择房间并加入", foreground="blue"
        )
    ...
    def refresh_rooms(self):
        ...
        try:
            ...
            self.lobby_status_label.config(
                text=f"已加载 {len(response.rooms)} 个房间", foreground="green"
            )
    ...
    def start_chat_session(self):
        ...
        def client_message_generator():
            """生成客户端消息的生成器"""
            # 首先发送加入请求
            join_request = chat_pb2.JoinRequest(
                user_name=self.user_name, room_id=self.current_room
            )
                    ...
                    if server_message.HasField("join_response"):
                        ...
                        if join_resp.success:
                            ...
                            self.gui_message_queue.put(
                                ("join_success", join_resp.message)
                            )
                        else:
    ...
    def show_chat_window(self):
        ...
        # 标题
        title_label = ttk.Label(
            main_frame, text=f"🎉 房间: {self.current_room}", font=("Arial", 14, "bold")
        )
        ...
        self.chat_text = scrolledtext.ScrolledText(
            chat_frame, wrap=tk.WORD, font=("Arial", 10), state=tk.DISABLED
        )
        ...
        # 发送按钮
        ttk.Button(input_frame, text="发送", command=self.send_message).grid(
            row=0, column=1
        )
        ...
        # 控制按钮
        ttk.Button(button_frame, text="离开房间", command=self.leave_room).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(button_frame, text="清空聊天记录", command=self.clear_chat).pack(
            side=tk.LEFT
        )
            ...
            while True:
                ...
                if msg_type == "join_success":
                    ...
                    self.add_chat_message(
                        f"[{timestamp}] {data.sender_name}: {data.text}"
                    )
                elif msg_type == "user_joined":
                    self.add_system_message(
                        f"🟢 欢迎 {data.user_name} 加入房间！当前在线人数：{data.current_count} 人"
                    )
                elif msg_type == "user_left":
                    self.add_system_message(
                        f"🔴 {data.user_name} 离开了房间，当前在线人数：{data.current_count} 人"
                    )
                    # 如果是自己离开，则停止聊天活动
    ...
    def leave_room(self):
        ...
        if self.chat_active and self.current_room:
            # 发送离开请求
            leave_request = chat_pb2.LeaveRequest(
                user_name=self.user_name, room_id=self.current_room
            )
